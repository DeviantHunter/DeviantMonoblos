...
namespace MapMagic
{
	public class MapMagicWindow : EditorWindow
	{
		...
		public void DrawPopup ()
		{
			...
			{
				Generator gen = gens.list[i];
				if (gen.guiRect.Contains(mousePos) && !(gen is Group)) clickedGenerator = gens.list[i];
			}
			...
			{
				Generator gen = gens.list[i];
				if (gen.guiRect.Contains(mousePos) && gen is Group) clickedGroup = gens.list[i] as Group;
			}
